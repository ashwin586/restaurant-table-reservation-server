import { findPartner, savePartner } from "../../repositories/partnerRepository.js";
...
export const verifyRegister = async(data) => {
    try{
        ...
        if(existingPartner) throw new Error('This Phone Number is already registered');
...
export const verifyLogin = async(data) => {
    try{
        ...
        const passwordCheck = await matchPassword(data.password, existingPartner.password);
        if(passwordCheck){
